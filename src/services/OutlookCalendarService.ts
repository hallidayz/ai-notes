...
import { CalendarService, type Meeting, type CalendarCredentials, type CalendarProvider } from './CalendarService.ts';
...
export class OutlookCalendarService extends CalendarService {
    ...
    private async exchangeCodeForTokens(code: string): Promise<any> {
        ...
        const apiUrl = (typeof import.meta.env !== 'undefined' ? import.meta.env.DEV : false)
            ? '/api/oauth/outlook/token'  // Vite proxy handles this
            : `${(typeof import.meta.env !== 'undefined' ? import.meta.env.VITE_API_URL : null) || 'http://localhost:4000'}/api/oauth/outlook/token`;
    ...
    async refreshTokenIfNeeded(): Promise<void> {
        ...
        try {
            ...
            const apiUrl = (typeof import.meta.env !== 'undefined' ? import.meta.env.DEV : false)
                ...
                : `${(typeof import.meta.env !== 'undefined' ? import.meta.env.VITE_API_URL : null) || 'http://localhost:4000'}/api/oauth/outlook/refresh`;
            const response = await fetch(apiUrl, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
