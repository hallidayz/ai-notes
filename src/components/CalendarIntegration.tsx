...
export const CalendarIntegration: React.FC<CalendarIntegrationProps> = ({ showStatus }) => {
  ...
  const fetchEvents = useCallback(async () => {
    setIsLoading(true);
    try {
      const connResponse = await fetch('/api/calendar/connections');
      if (connResponse.ok) {
        const conns = await connResponse.json();
        setConnections(conns);
        setConnectedProviders(conns.map((c: CalendarConnection) => c.provider));
      }
  ...
  const handleConnect = async (provider: string) => {
    ...
    try {
      // Use Nylas for Google and Microsoft if configured, otherwise fallback to direct
      const useNylas = ['google', 'microsoft'].includes(provider);
      const endpoint = useNylas ? `/api/auth/nylas/url?provider=${provider}` : `/api/auth/${provider}/url`;
  ...
  const handleAppleConnect = async (e: React.FormEvent) => {
    ...
    try {
      await fetch('/api/calendar/connections', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ provider: 'apple', tokens: appleConfig })
      });
      fetchEvents();
  ...
  return (
    <div className="calendar-integration p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
        {providers.map((p) => {
          const conn = connections.find((c: CalendarConnection) => c.provider === p.id);
